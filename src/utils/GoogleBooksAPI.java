...
public class GoogleBooksAPI {
    ...
    private JSONObject getBookDetails(JSONObject response) {
        if(!response.getJSONArray("items").isEmpty()) {
            return response.getJSONArray("item").getJSONObject(0).getJSONObject("volumeInfo");
        }
        return null;
    }
    ...
    public Book getBookByISBN(String isbn) {
        ...
        if(volumeInfo != null) {
            ...
            String imageLink = volumeInfo.optJSONObject("imageLinks") != null ?
                    volumeInfo.getJSONObject("imageLinks").optString("thumbnail", "No Image Link") : "No Image Link";
